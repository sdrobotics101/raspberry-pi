...
std::vector < Contour > ContourDetector::detect(cv::Mat image)
{
	...
	cv::imwrite("threshold_out.jpg", threshold_out);
	...
	cv::imwrite("canny_out.jpg", canny_out);
	std::vector < std::vector < cv::Point > >all_contours_raw;
	cv::findContours(canny_out, all_contours_raw, CV_RETR_LIST,
			 CV_CHAIN_APPROX_SIMPLE);
	cv::Mat find_contours_out = image.clone();
	cv::drawContours(find_contours_out, all_contours_raw, -1, cv::Scalar(0, 0, 0), 5);
	cv::imwrite("find_contours_out.jpg", find_contours_out);
